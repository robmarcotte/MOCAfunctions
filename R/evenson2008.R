...
evenson2008 = function(acc_data_counts, sed_cp = 26, mpa_cp = 574, vpa_cp = 1003, epoch = 15, expand_1sec = F){
  if(epoch != 15){
    stop("Evenson2008 Cutpoint was developed using 15-second epochs. As of now, cutpoint scaling is not supported.")
  }

  acc_data_new = ag_epochr(acc_data_counts, epoch = epoch)

  acc_data_new$Evenson2008 = factor(cut(acc_data_new$Axis1, breaks = c(-Inf, sed_cp, mpa_cp, vpa_cp, Inf), labels = c('Sedentary','LPA','MPA','VPA'), right = F), levels = c('Sedentary','LPA','MPA','VPA'), labels = c('Sedentary','LPA','MPA','VPA'))

  if(expand_1sec == T){
    Evenson2008 = data.frame(Timestamp = acc_data_counts$Timestamp,
                              Axis1_15sec = rep(acc_data_new$Axis1, each = epoch)[1:nrow(acc_data_counts)],
                              Evenson2008 = factor(rep(acc_data_new$Evenson2008, each = epoch), levels =c('Sedentary','LPA','MPA','VPA'), labels =c('Sedentary','LPA','MPA','VPA'))[1:nrow(acc_data_counts)])
    return(Evenson2008)
