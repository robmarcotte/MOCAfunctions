...
crouter2012_VM = function(acc_data_counts, epoch = 10, expand_1sec = F){
  ...
  acc_data_new$cv1 = slide_dbl(acc_data_new$VM, function(x){ifelse(mean(x) == 0, 0, sd(x)/mean(x))}, .before = 0, .after = 5, .complete = T)
  acc_data_new$cv2 = slide_dbl(acc_data_new$VM, function(x){ifelse(mean(x) == 0, 0, sd(x)/mean(x))}, .before = 1, .after = 4, .complete = T)
  acc_data_new$cv3 = slide_dbl(acc_data_new$VM, function(x){ifelse(mean(x) == 0, 0, sd(x)/mean(x))}, .before = 2, .after = 3, .complete = T)
  acc_data_new$cv4 = slide_dbl(acc_data_new$VM, function(x){ifelse(mean(x) == 0, 0, sd(x)/mean(x))}, .before = 3, .after = 2, .complete = T)
  acc_data_new$cv5 = slide_dbl(acc_data_new$VM, function(x){ifelse(mean(x) == 0, 0, sd(x)/mean(x))}, .before = 4, .after = 1, .complete = T)
  acc_data_new$cv6 = slide_dbl(acc_data_new$VM, function(x){ifelse(mean(x) == 0, 0, sd(x)/mean(x))}, .before = 5, .after = 0, .complete = T)
  ...
  for(i in 1:nrow(acc_data_new)){
    ...
    if(acc_data_new$VM[i] <= 75){
      acc_data_new$METs[i] = 1.0
    } else {
      acc_data_new$METs[i] = ifelse(acc_data_new$CV[i] <= 25,
                                 0.0137*(exp(.848*ln(acc_data_new$VM[i]))),
                                 1.219-(.145*ln(acc_data_new$VM[i]))-(.0586*(ln(acc_data_new$VM[i])^2))+(.0229*(ln(acc_data_new$VM[i])^3)))
    }
  }
