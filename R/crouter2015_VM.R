...
crouter2015_VM = function(acc_data_counts, sed_cp = 101, mpa_cp = 610, vpa_cp = 1810, epoch = 5, expand_1sec = F){
  ...
  acc_data_new$Crouter2015_VM = factor(cut(acc_data_new$VM_LFE, breaks = c(-Inf, sed_cp, mpa_cp, vpa_cp, Inf), labels = c('Sedentary','LPA','MPA','VPA'), right = F), levels = c('Sedentary','LPA','MPA','VPA'), labels = c('Sedentary','LPA','MPA','VPA'))

  if(expand_1sec == T){
    Crouter2015_VM = data.frame(Timestamp = acc_data_counts$Timestamp,
                              Axis1_60sec = rep(acc_data_new$VM_LFE, each = epoch)[1:nrow(acc_data_counts)],
